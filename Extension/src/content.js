...
let addToggleListener = (
 ...
 function addToBookmarkedFolder(chat, isFromStorage = false) {
   ...
   if (bookmarkedChatContainer) {
     ...
     let existingBookmarkedIcon =
       bookmarkAdd.querySelector("._bookmarked_icon");
 ...
 let removeBookmarkListeners = () => {
   ...
   chats.forEach((chat) => {
     ...
     if (bookmarkAdd && bookmarkAdd.toggleListener) {
       ...
       if (bookmarkedIcon) {
         bookmarkedIcon.removeEventListener(
           "click",
 ...
 async function nCleanChatAndAppend(folder_div, chat) {
   ...
   if (folderContent) {
     ...
     doCheckingAndStoring(folderId, cloneChat);
   }
 }
 ...
 let addIconsToChat = () => {
   ...
   chats.forEach((chat) => {
     ...
     if (bookmarkAdd) {
       // The bookmarked Icon
       let bookmarkedIcon = document.createElement("img");
       bookmarkedIcon.src = chrome.runtime.getURL(
         "../images/bookmark_fill.png"
       );
       bookmarkedIcon.alt = "Bookmarked Icon";
       bookmarkedIcon.classList.add("_bookmarked_icon", "_cF_GNewDTGpqsqNfG");
 ...
 function handleDrop(e) {
   ...
   clonedElement.id = `draggable-${Math.random()
     .toString(36)
     .substring(2, 11)}`; // Assign a new unique ID to the cloned element
 ...
 let doCheckingAndStoring = (folderId, cloneChat) => {
   let chatData = {
     content: cloneChat.outerHTML,
     chat_id: cloneChat.id,
   };
 ...
 const doChatRemovalCheck = (folderId, chatId) => {
   chrome.storage.local.get(
     ...
     async (result) => {
       ...
       if (isLoggedIn && userId && !userHasPayed && !hasAccess) {
         console.log("The chat to remove: ", chatId);
         console.log("The folder to remove from: ", folderId);
   ...
   chrome.storage.sync.get(["folders"], async (result) => {
     ...
     if (folder) {
       ...
       if (chatIndex > -1) {
         ...
         console.log("The folder before saving: ", folder);
